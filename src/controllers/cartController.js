...
const  { isValidObjectId, isValidRequestBody, isValid} = require('../validators/validator');
...
const addToCart = async (req, res) => {
     ...
     try {
        ...
        if ('cartId' in data) {
            if(! isValid(cartId)) return res.status(400).send({ status: false, message: 'Cart Id value should be present' });
        }
        ...
        const productDoc = await productModel.findOne({ _id : productId , isDeleted: false }).lean().select({price: 1}) 
        //console.log(productDoc)
        if( productDoc === null ) {
            return res.status(404).send({ status: false, message: 'Product does not exist or may be deleted' });
        }
        ...
        if(cartId && cartDoc._id.toString() !== cartId) {
            return res.status(400).send({ status: false, message: 'cartId does not belong to userId' });
        }
        ...
        if( productIdList.find( (pId) => pId.toString() == productId ) ){
            ...
            const updatedCart = await cartModel.findOneAndUpdate( { userId : userId , 'items.productId': productId },
                {$inc : { totalPrice : +productDoc.price,
                          totalItems : +1,
                          'items.$.quantity' : +1
                        }
                }, { new : true} )

                return res.status(200).send({status: true, message : "Cart details" , data: updatedCart })
        ...
        const updatedCart = await cartModel.findOneAndUpdate ({ userId : userId }, 
            ...
            return res.status(201).send({status: true, message : "Cart details" , data: updatedCart })
...
const updateCart = async (req, res) => {
     try {
        ...
        if(! cart || (cart.items.length == 0 && cart.totalItems == 0 && cart.totalPrice == 0 )) {
            return res.status(404).send({ status: false, message: 'Cart does not exist or may be deleted' });
        }
        ...
        const isProduct = await productModel.findOne({_id: productId, isDeleted: false}).lean() 
        ...
        if (isNaN(removeProduct)) {
            return res.status(400).send({ status: false, message: "Enter the number as value for removeProduct" })
        ...
        let productIdList = itemList.map( (item) => item = item.productId.toString())
        ...
        if(removeProduct == 0 || (removeProduct == 1 && itemList[pIndex].quantity == 1) ) {

            let productPrice = itemList[pIndex].quantity * isProduct.price

            const updated_Cart = await cartModel.findOneAndUpdate( {userId : userId},
                {
                    $pull : { items : {productId : productId} },
                    $inc : { totalPrice : - productPrice , totalItems : -itemList[pIndex].quantity}
                
                }, {new : true} )

            return res.status(200).send({status: true , message: 'sucessfully Removed product', data: updated_Cart})

        }
        
        if(removeProduct == 1) {
            
            const updated_Cart = await cartModel.findOneAndUpdate( {userId : userId, "items.productId": productId},
                { 
                    $inc : { totalPrice : -isProduct.price , totalItems : -1 , "items.$.quantity" : -1} 
